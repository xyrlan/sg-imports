...
const UserHeader = ({ t, user, onLogout }: { t: TFn; user: UserInfo; onLogout: () => void }) => (
  <Card variant="secondary" className="w-full"><Card.Content>
    <div className="flex items-center justify-between p-4">
      <div className="flex items-center gap-3"><div className="rounded-full bg-primary/20 p-2"><User className="text-primary w-4 h-4" /></div><div className="flex flex-col"><span className="text-sm font-medium text-foreground">{user.name || 'Usuário'}</span><span className="text-xs text-muted">{user.email}</span></div></div>
      <Button variant="danger" size="sm" onClick={onLogout}><LogOut className="w-4 h-4 mr-1" />{t('userHeader.logout')}</Button>
    </div>
    {!user.emailConfirmed && (<div className="px-4 pb-4"><div className="pt-3 border-t border-border"><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-warning" /><span className="text-xs text-foreground/90">{t('userHeader.pendingVerification')}</span></div></div></div>)}
  </Card.Content></Card>
);

const LoadingSkeleton = () => (<Card variant="secondary" className="w-full"><Card.Content><div className="flex flex-col items-center gap-4 py-8"><Skeleton className="h-12 w-12 rounded-full" /><div className="space-y-2 w-3/4"><Skeleton className="h-4 w-full rounded" /><Skeleton className="h-3 w-4/5 rounded" /></div></div></Card.Content></Card>);
const LoadingState = ({ t }: { t: TFn }) => (<Card variant="secondary" className="w-full"><Card.Content><div className="flex flex-col items-center justify-center gap-4 py-8"><span className="inline-flex shrink-0 size-4 items-center justify-center"><Spinner color="current" size="sm" className="size-4!" /></span><p className="text-sm text-muted">{t('states.verifying.title')}</p></div></Card.Content></Card>);
const VerifyingState = ({ t }: { t: TFn }) => (<Card variant="secondary" className="w-full"><Card.Content><div className="text-center py-8 space-y-4"><span className="inline-flex shrink-0 size-4 items-center justify-center"><Spinner color="current" size="sm" className="size-4!" /></span><h1 className="text-xl font-semibold">{t('states.verifying.title')}</h1><p className="text-sm text-foreground/90">{t('states.verifying.description')}</p></div></Card.Content></Card>);
const SuccessState = ({ t }: { t: TFn }) => (<Card variant="secondary" className="w-full"><Card.Content><div className="text-center py-8 space-y-4"><div className="flex justify-center"><div className="rounded-full bg-success/20 p-4"><CircleCheck className="text-success" size={48} /></div></div><h1 className="text-xl font-semibold text-success">{t('states.verified.title')}</h1><p className="text-sm text-foreground/90">{t('states.verified.description')}</p><div className="flex items-center justify-center gap-2 text-xs text-muted mt-2"><Spinner size="sm" /><span>{t('states.verified.redirecting')}</span></div></div></Card.Content></Card>);
...
function EmailDisplay({ email }: { email?: string }) { if (!email) return null; return (<div className="text-xs text-muted bg-border p-3 rounded-lg mt-2"><Mail className="inline w-4 h-4 mr-1" />{email}</div>); }
...
const ErrorState = ({ t, error, onResend, resendLoading, resendCooldown, userEmail }: { t: TFn; error: VerificationError; onResend?: () => void; resendLoading?: boolean; resendCooldown: number; userEmail?: string }) => (
  <Card variant="secondary" className="w-full"><Card.Content><div className="text-center py-8 space-y-4"><div className="flex justify-center"><div className="rounded-full bg-danger/20 p-4"><CircleX className="text-danger" size={48} /></div></div><h1 className="text-xl font-semibold text-danger">{t('states.error.title')}</h1><p className="text-sm text-foreground/90">{error.message}</p><EmailDisplay email={userEmail} />{error.canRetry && onResend && (<div className="space-y-3 mt-4"><p className="text-sm text-foreground/90">{t('states.error.canRetry')}</p><ResendBtn t={t} onResend={onResend} resendLoading={resendLoading} resendCooldown={resendCooldown} /></div>)}</div></Card.Content></Card>
);
...
const WaitingState = ({ t, userEmail, onResend, resendLoading, resendCooldown }: WaitingProps) => (
  <Card variant="secondary" className="w-full"><Card.Content><div className="text-center py-8 space-y-4"><div className="flex justify-center"><div className="rounded-full bg-primary/20 p-4"><Mail className="text-primary" size={48} /></div></div><h1 className="text-xl font-semibold">{t('states.waiting.title')}</h1><p className="text-sm text-foreground/90">{t('description')}</p><EmailDisplay email={userEmail} /><div className="space-y-3 mt-4"><p className="text-xs text-muted">{t('states.waiting.noReceived')}</p><ResendBtn t={t} onResend={onResend} resendLoading={resendLoading} resendCooldown={resendCooldown} /></div></div></Card.Content></Card>
);
const TokenExpiredState = ({ t, userEmail, onResend, resendLoading, resendCooldown }: WaitingProps) => (
  <Card variant="secondary" className="w-full"><Card.Content><div className="text-center py-8 space-y-4"><div className="flex justify-center"><div className="rounded-full bg-warning/20 p-4"><Clock className="text-warning" size={48} /></div></div><h1 className="text-xl font-semibold text-warning">{t('states.tokenExpired.title')}</h1><p className="text-sm text-foreground/90">{t('states.tokenExpired.description')}</p><EmailDisplay email={userEmail} /><div className="space-y-3 mt-4"><p className="text-xs text-muted">{t('states.tokenExpired.checkInbox')}</p><ResendBtn t={t} onResend={onResend} resendLoading={resendLoading} resendCooldown={resendCooldown} /></div></div></Card.Content></Card>
);
