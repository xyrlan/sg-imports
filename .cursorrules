...
- **Hero UI Table Limitation:** Hero UI v3 beta 6 lacks a native Table component.
- **Database:** PostgreSQL (Supabase) + Drizzle ORM (`postgres.js` driver).
- **Workflows:** Inngest for durable execution and complex sagas.

## 🧩 Engineering Principles & Clean Code

- **Componentization:** Break down UI into small, reusable components. If a component exceeds 100 lines, evaluate if it should be split.
- **Single Responsibility (SRP):** Each function, server action, or component should do one thing well.
- **Clean Code:** Use descriptive variable names (e.g., `isShipmentOverdue` instead of `check`). Avoid deeply nested conditionals (use early returns).
- **DRY (Don't Repeat Yourself):** Abstract shared logic into `@/src/hooks` or `@/src/lib/utils`.
- **Props Typing:** Strictly type all component props using TypeScript interfaces.
...
## ⚡ Inngest & Workflow Rules (CRITICAL)
...
- **Proactive:** After writing or modifying code, use the `sg-imports-compliance` subagent to review changes against project rules. Do not wait for the user to ask.
